...
const slashCommands: SlashCommand[] = [
    {
        label: "Обычный текст",
        description: "Начать простой абзац",
        icon: Text,
    ...
    },
    {
        ...
        icon: Heading1,
        run: (editor) => editor.chain().focus().toggleHeading(
            {level: 1}
        ).run()
    },
    {
    ...
    },
    {
        ...
        label: "Список задач",
        description: "Интерактивные чекбоксы",
        icon: ListChecks,
        run: (editor) => editor.chain().focus().toggleTaskList().run()
    }, {
        label: "Цитата",
        description: "Выделить важную мысль",
        icon: Quote,
        ...
        label: "Разделитель",
        description: "Разделить смысловые блоки",
        icon: Minus,
        ...
        label: "Жирный текст",
        description: "Выделить важное",
        icon: Bold,
        run: (editor) => editor.chain().focus().toggleBold().run()
    }, {
        label: "Курсив",
        description: "Добавить акцент",
        icon: Italic,
        run: (editor) => editor.chain().focus().toggleItalic().run()
    }, {
        label: "Зачёркнутый текст",
        description: "Отметить неактуальное",
        icon: Strikethrough,
        run: (editor) => editor.chain().focus().toggleStrike().run()
    }, {
        label: "Строчный код",
        description: "Выделить команду или код",
        icon: Code2,
...
export function NoteEditor({
    ...
    const editor = useEditor({
        ...
        extensions: [
            ...
            LinkExtension.configure(
                {autolink: true, openOnClick: false}
            ),
            TaskList,
            TaskItem.configure(
                {nested: true}
        ...
        onUpdate: (
            ...
            if (! replacingContent.current) 
                onUpdate({content: currentEditor.getMarkdown()});
    ...
    useEffect(() => {
        ...
        function closeSlashMenu(event : PointerEvent) {
            if (!(event.target instanceof Element) || ! event.target.closest(".project-slash-menu")) 
                setSlashMenu(null);
        ...
        <div className="flex h-full min-h-0 flex-1 flex-col overflow-hidden bg-background">
            <header className="flex h-11 shrink-0 items-center justify-between gap-3 border-b px-3">
                <div className="flex min-w-0 items-center gap-1">
                    {
                    !isNotesPanelOpen && (
                        <Button aria-label="Показать список заметок"
                        ...
                        <DropdownMenuContent align="end" className="w-52">
                            ...
                            note.deletedAt ? (
                                <>
                                    ...
                                    <DropdownMenuSeparator/>
                                    <DropdownMenuItem onSelect={onDeletePermanently}
                                        variant="destructive">
                                ...
                                <>
                                    ...
                                    <DropdownMenuSub>
                                        <DropdownMenuSubTrigger><BookOpen/>В блокнот</DropdownMenuSubTrigger>
                                        <DropdownMenuSubContent>
            ...
            <main className="min-h-0 flex-1 overflow-y-auto">
                <div className="mx-auto w-full max-w-5xl px-10 pb-32 pt-10 max-sm:px-5">
                    <input aria-label="Заголовок заметки" className="w-full bg-transparent text-4xl font-semibold tracking-tight outline-none placeholder:text-muted-foreground/60"
                        onChange={
                            (event) => onUpdate({title: event.target.value})
                        }
                    ...
                    editor && (
                        <>
                            ...
                            <EditorContent className="project-tiptap-editor mt-8"
                                editor={editor}/>
                        </>
                    )
                } </div>
            ...
            slashMenu && (
                <div className="project-slash-menu"
                    ...
                    <div className="border-b px-3 py-2">
                        ...
                        {
                        slashMenu.query && <span className="mt-0.5 block truncate text-xs">/{
                            slashMenu.query
                        }</span>
                    } </div>
                    <div className="p-1">
                        ...
                        visibleCommands.length ? visibleCommands.map((command, index) => {
                            ...
                            return (
                                <button className="project-slash-command"
                                    data-selected={
                ...
                <DialogContent>
                    <form onSubmit={
                        ...
                        <div className="grid gap-2 py-5">
                            ...
                            <Input autoFocus id="note-link-url"
                                onChange={
